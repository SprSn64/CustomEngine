...
Texture* loadRawTexture(char* path){
	...
	char* fuck = fgets(fileData, fileSize, file); (void)fuck;

	Texture* newTex = newTexture((fileData[0] << 8) + fileData[1], (fileData[2] << 8) + fileData[3]);

	memcpy(newTex->pixels, &fileData[4], newTex->width*newTex->height * sizeof(Uint32));
...
Texture* loadTexture(char* path){
	...
	Texture* newTex = newTexture(newSurface->w, newSurface->h);
...
void drawTexture(Texture* target, Texture* tex, SDL_Rect* source, SDL_Rect* dest, Uint32 colour){
	...
	for(Uint32 i=0; i<(Uint32)abs(dest->w) * abs(dest->h); i++){
		...
		if(currPixel >> 24 > 0)
			setPixel(target, dest->x + i % abs(dest->w), dest->y + (i / abs(dest->w)), (colour == 0xFFFFFFFF) ? currPixel : colourMult(currPixel, colour));
	}
}
